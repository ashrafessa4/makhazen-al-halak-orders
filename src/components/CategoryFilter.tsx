...
const CategoryFilter = ({ categories, selectedCategory, onCategorySelect }: CategoryFilterProps) => {
  return (
    <div className="flex justify-center mb-8 sm:mb-10">
      <div className="flex flex-wrap gap-2 justify-center bg-white/90 backdrop-blur-sm p-3 rounded-2xl shadow-lg border-2 border-gray-100">
        {categories.map((category) => (
          <Badge
            key={category}
            variant={selectedCategory === category ? "default" : "outline"}
            className={`cursor-pointer px-3 sm:px-4 py-2 text-xs sm:text-sm font-semibold transition-all duration-200 transform hover:scale-105 ${
